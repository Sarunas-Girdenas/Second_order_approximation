...
'Translated to Python by Sarunas Girdenas and Hyun Changi Yi, 2014'
...
GAMA, DELTA, ALFA, RHO, BETTA = smp.symbols('GAMA, DELTA, ALFA, RHO, BETTA')

# Define Variables

c, cp, k1, k1p, k2, k2p, a1, a1p, a2, a2p = smp.symbols('c, cp, k1, k1p, k2, k2p, a1, a1p, a2, a2p')

# Write model equations

f1 = c**(-GAMA) - BETTA * cp**(-GAMA) * (ALFA * a1p * k1p**(ALFA-1) + 1 - DELTA)

f2 = c**(-GAMA) - BETTA * cp**(-GAMA) * (ALFA * a2p * k2p**(ALFA-1) + 1 - DELTA)

f3 = 2 * c + k1p - (1-DELTA) * k1 + k2p - (1-DELTA) * k2 - a1 * k1**(ALFA) - a2 * k2**(ALFA)

f4 = smp.log(a1p) - RHO * smp.log(a1)

f5 = smp.log(a2p) - RHO * smp.log(a2)

# Create function f
F = smp.Matrix([f1, f2, f3, f4, f5])

# Define the vector of controls, y, and states, x
x  = smp.Matrix([k1, k2, a1, a2])
y  = smp.Matrix([c]) 
xp = smp.Matrix([k1p, k2p, a1p, a2p])
yp = smp.Matrix([cp])

# Make f a function of the logarithm of the state and control vector

F = F.subs([(k1,smp.exp(k1)),(k2,smp.exp(k2)),(a1,smp.exp(a1)),(a2,smp.exp(a2)),(c,smp.exp(c)),(k1p,smp.exp(k1p)),(k2p,smp.exp(k2p)),(a1p,smp.exp(a1p)),(a2p,smp.exp(a2p)),(cp,smp.exp(cp))])
...
ALFA = 0.3 # capital elasticity of substitution
RHO = 0 # serial correlation of productivity shock
BETTA = 0.95 # yet another discount factor
...
Parameters       = ['GAMA', 'DELTA', 'ALFA', 'RHO', 'BETTA']

Param_Values     = [2, 0.1, 0.3, 0, 0.95]
...
Variables         = ['c', 'cp', 'k1', 'k1p', 'k2', 'k2p', 'a1', 'a1p', 'a2', 'a2p']

SS_Variables      = [c, cp, k1, k1p, k2, k2p, a1, a1p, a2, a2p] # steady state values of variables
